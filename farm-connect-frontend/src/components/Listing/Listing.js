...
const Listing = (props) => {
        ...
        return (
          <>
            <button
              ...
              onClick={() => props.history.push(`/listings/${listingId}/edit`)}
            >
              Edit
            </button>
          </>
  ...
  const handleUpdateUserListingInterest = (
    currentUserId,
    listingUserId,
    listingId,
    listingInterests
    ...
    const interest = listingInterests.find(
      (interest) => interest.user_id === parseInt(currentUserId)
    );

    if (parseInt(currentUserId) !== listingUserId) {
      interest
        ? props.removeUserListingInterest(listingId, interest.id, currentUserId)
        : props.addUserListingInterest(currentUserId, listingId);
    } else {
      // do nothing
    }
  };
  ...
  props.userAttributes.listings.length > 1
    ? (listingPluralized = "My Listings")
    : (listingPluralized = "My Listing");
          ...
          {props.listing.id && (
            <>
              <div id="listing-details">
                ...
                <p>
                  <label className="listing-label-color listing-label-padding">
                    <strong>LID:</strong>{" "}
                  </label>
                  <span className="listing-id-span">
                    {padIds(props.listing.id)}
                  </span>
                </p>
                ...
                <p>
                  ...
                  <Link
                    id="listing-name-link"
                    to={`/farmers/${props.listing.attributes.user.id}`}
                    onClick={(e) =>
                      handleFetchFarmer(e, props.listing.attributes.user.id)
                    }
                  >
                    {getFullName(
                      props.listing.attributes.user.first_name,
                      props.listing.attributes.user.last_name
                    )}
                  </Link>
                </p>
                ...
                <p>
                  ...
                  {props.listing.attributes.availability
                    ? getDate(props.listing.attributes.availability)
                    : null}
                </p>
                ...
                {props.listing.attributes.quantity ? (
                  <p>
                    ...
                    {props.listing.attributes.quantity}{" "}
                    {props.listing.attributes.quantity > 1
                      ? props.listing.attributes.measure + "s"
                      : props.listing.attributes.measure}
                  </p>
                ) : null}

                {props.listing.attributes.information ? (
                  <>
                    <label className="listing-label-color">
                ...
                <p className="no-select">
                  <label
                    ...
                    style={{
                      color: setHeartColor(
                        props.userId,
                        props.listing.attributes.interests
                      ),
                    }}
                    onClick={() =>
                      handleUpdateUserListingInterest(
                        props.userId,
                        props.listing.attributes.user.id,
                        props.listing.id,
                        props.listing.attributes.interests
                      )
                    }
                ...
                {props.listing.attributes.closed ? (
                  <h5 id="closed-listing">
                    Closed on {getDateTime(props.listing.attributes.closed)}
                  </h5>
                ) : parseInt(props.userId) ===
                  props.listing.attributes.user.id ? (
                  <h5 id="listing-last-edit">
                    Last edited on{" "}
                    {getDateTime(props.listing.attributes.updated_at)}
                  </h5>
                ) : null}
                ...
                <div id="listing-btn-div">
                  {props.userAttributes.type === "Farmer" ? (
                    <button
                      id="listing-mylisting-btn"
                      className="global-btn"
                      onClick={handlePushToUserListings}
                    >
                      {listingPluralized}
                    </button>
                  ) : null}
                  <button
                    id="listing-listings-btn"
                    className="global-btn"
                    onClick={handlePushToListings}
                  >
                    Listings
                  </button>
                </div>
