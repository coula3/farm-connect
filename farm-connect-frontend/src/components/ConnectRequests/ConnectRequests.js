            ...
            <div className="connect_requests_card">
                ...
                <ul id="connect_requests_ul">
                    {totalConnectRequests.map(connect => 
                        <li id="connect_li" key={connect[0].id}><Link to={`/${connect[3].toLowerCase() + "s"}/${connect[0].user_id}`} onClick={() => fetchProfile(connect[0].user_id, connect[3])}>{connect[1]} {connect[2]}</Link></li>)
                }
                </ul>
            </div>
        </div>
