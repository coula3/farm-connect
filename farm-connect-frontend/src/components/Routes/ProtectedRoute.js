...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetchListings: (farmer, routerProps) =>
      dispatch(listingsActions.fetchListings(farmer)),
    removeUserListingInterest: (listingId, interestId, currentUserId) =>
      dispatch(
        listingsActions.removeUserListingInterest(
          listingId,
          interestId,
          currentUserId
        )
      ),
    addUserListingInterest: (currentUserId, listingId) =>
      dispatch(
        listingsActions.addUserListingInterest(currentUserId, listingId)
      ),
    requestConnect: (currentUserId, connectId) =>
      dispatch(requestConnect(currentUserId, connectId)),
    acceptConnect: (currentUserId, connectId) =>
      dispatch(acceptConnect(currentUserId, connectId)),
    unConnectUsers: (currentUserId, connectId) =>
      dispatch(unConnectUsers(currentUserId, connectId)),
