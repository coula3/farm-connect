...
class EditListing extends Component {
  ...
  handleSubmit = (e) => {
    ...
    const listing = this.props.listingsInterests.find(
      (listing) => listing[0] === parseInt(this.props.listing.id)
    );
  ...
  getAvailability = (availabilityProps) => {
    return this.state.editMode
      ? this.state.listing.availability.slice(0, 10)
      : availabilityProps.slice(0, 10);
  };
  ...
  getAvailable = (availableProps) => {
    return this.state.editMode
      ? this.state.listing.available === "Yes" ||
        this.state.listing.available === "No"
  ...
  getCharactersLength = (infoProps) => {
    return this.state.editMode
      ? this.state.listing.information.length
      : infoProps.length;
  };
  ...
  closeListingWarning = () => {
    return this.state.listing.closed
      ? "WARNING: closed listing cannot be edited"
      : null;
  };
  ...
  render() {
    ...
    const stringCurrentDate =
      ...
      currentDate.getDate();
    const maxXterColor =
      this.state.maxInfoCharacters -
        this.getCharactersLength(this.props.listing.attributes.information) <=
      25
        ? "xter-critical-color"
        : null;
    const warningMsgStyles = this.state.listing.closed
      ? "listing-close-warning-span"
      : null;
          ...
          <div className="edit-listing-card">
            ...
            <span id="edit-listing-id-span" className="tooltip">
              ...
              <span className="tooltiptext">
                Listing Date: {getDate(this.props.listing.attributes.date)}
              </span>
            </span>
            ...
            <EditListingForm
              ...
              closeListing={(stringCurrentDate) =>
                this.closeListing(stringCurrentDate)
              }
...
const mapDispatchToProps = (dispatch, routerProps) => {
  return {
    editListing: (listingId, payload, fetchInterestsListings) =>
      dispatch(
        editListing(listingId, payload, fetchInterestsListings, routerProps)
      ),
