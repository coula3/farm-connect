...
class Listings extends React.Component {
  ...
  stageApplication = () => {
    ...
    !this.props.countUserInterestsListings &&
  ...
  switchListingsType = () => {
    if (
      !this.props.openListingsRendered &&
      (this.props.match.path === "/listings" ||
        this.props.match.path === "/listings/other-farmers" ||
        this.props.match.path === "/users/:id/listings")
    ) {
      this.stageApplication();
    } else if (
      !this.props.myInterestsRendered &&
      this.props.match.path === "/listings/my-interests"
    ) {
      this.props.fetchUserInterestsListings(this.props.userId);
    } else if (
      (this.props.openListingsRendered || this.props.myInterestsRendered) &&
  ...
  render() {
    ...
    if (this.props.match.path === paths().USER_LISTINGS_PATH) {
      ...
      baseListings = this.props.listings.filter(
        (listing) =>
          listing.attributes.user_id === parseInt(this.props.match.params.id)
      );
    ...
    if (this.state.isAvailableSorted) {
      sortedListings = [...baseListings].sort(
        (a, b) => b.attributes.available - a.attributes.available
      );
    } else {
    ...
    if (
      ...
      listings = [...sortedListings].filter(
        (listing) =>
          listing.attributes.date.includes(searchText) ||
          listing.attributes.commodity.name
            .toLowerCase()
            .includes(searchText) ||
          (
            listing.attributes.user.first_name +
            " " +
            listing.attributes.user.last_name
          )
            .toLowerCase()
            .includes(searchText)
      );
      ...
      listings = [...sortedListings].filter(
        (listing) =>
          listing.attributes.date.includes(searchText) ||
          listing.attributes.commodity.name.toLowerCase().includes(searchText)
      );
    } else {
    ...
    const renderListings = listings.map((listing) => {
      ...
      const viewButtonColor =
        parseInt(this.props.userId) === listing.attributes.user.id
          ? "view-user-btn-color"
          : "view-btn-color";
      const dateDiff =
        (new Date(listing.attributes.date) -
          new Date(listing.attributes.closed)) /
        oneDay;
      const rowHighlight =
        listing.attributes.interests.length >= 5
          ? "listings-td-g"
          : "listings-td";
      ...
      renderLinkToFarmerProfile = (
        <div id="link-div">
          <Link
            ...
            onClick={(e) => this.handleFetchFarmer(e, userId)}
          >
            {getFullName(
              listing.attributes.user.first_name,
              listing.attributes.user.last_name
      ...
      const connectedToUser =
        this.props.userConnects &&
        this.props.userConnects
          .filter((connect) => connect[0].status === "accepted")
          .find(
            (connect) =>
              (listing.attributes.user_id !== parseInt(this.props.userId) &&
                listing.attributes.user_id === connect[0].user_id) ||
              (listing.attributes.user_id !== parseInt(this.props.userId) &&
                listing.attributes.user_id === connect[0].connect_id)
          );

      const renderConnectSymbol = connectedToUser ? (
        <span id="connect-span-global">
          <span id="connect-span1"></span>
          <span id="connect-span2"></span>
        </span>
      ) : null;
      ...
      return (
        <tr
          key={listing.id}
          id={rowHighlight}
          className="listings-th-td"
          onDoubleClick={() => {
            this.handleFetchListing(listing.id);
        ...
        >
          <td>
            <Link
              ...
              title="View Listing"
              onClick={() => this.handleFetchListing(listing.id)}
            >
              {padIds(listing.id)}
            </Link>
          </td>
          ...
          this.props.match.path === paths().MY_INTERESTS_PATH ? (
            <td>
              <Link
                to={`/farmers/${userId}/listings`}
                title={`${firstName}'s Listings`}
                onClick={() =>
                  this.handleFetchListings(listing.attributes.user_id)
                }
              >
                {this.getName(
                  listing.attributes.user.first_name,
          ...
          {this.props.match.path !== paths().USER_CLOSED_LISTINGS_PATH ? (
            <td id="td-available" className="tooltip">
              {available}
              <span className="tooltiptext">
                {listing.attributes.availability.slice(0, 10)}
              </span>
            </td>
          ) : null}

          <td className="listings-interests">
            {listing.attributes.interests.length > 0
              ? listing.attributes.interests.length
              : null}
          </td>
          ...
          {this.props.match.path === paths().USER_CLOSED_LISTINGS_PATH ? (
            <>
              <td>
                {listing.attributes.closed
                  ? listing.attributes.closed.slice(0, 10)
                  : null}
              </td>
          ...
          <td className="listings-view-btn">
            <button
              id={viewButtonColor}
          ...
          {this.props.match.path === paths().USER_LISTINGS_PATH ? (
            <td>
              <button
                id="listing-delete-btn"
                onClick={() => this.handleDeleteListing(listing.id)}
              >
                X
              </button>
            </td>
        ...
        <table className="listings-table">
          <thead>
            <tr className="listings-th-td">
              <th>LID</th>
              <th>
                <span className={this.getTableHeadSearchStyles()}>
                  List Date
                </span>
              </th>
              <th>
              ...
              {this.props.match.path !== paths().USER_CLOSED_LISTINGS_PATH ? (
                <th onClick={() => this.handleSortAvailable()}>
                  <span id={this.getAvailableStyles()}>Available</span>
                </th>
              ) : null}
    ...
    return (
      <div className="Listings-main-div">
        {this.props.isLoadingListings ? (
          <Loader />
        ) : this.props.userAttributes.listings.length === 0 &&
          (this.props.match.path === paths().USER_LISTINGS_PATH ||
            this.props.match.path === paths().USER_CLOSED_LISTINGS_PATH) ? (
          this.props.match.path === paths().USER_LISTINGS_PATH ? (
            <>
        ...
        ) : this.props.userAttributes.listings.every(
            (listing) => listing.closed === null
          ) && this.props.match.path === paths().USER_CLOSED_LISTINGS_PATH ? (
          <h4>No Closed Listing</h4>
        ) : (
          <>
            <div id="listings-header">
              ...
              this.props.match.path.endsWith(":id/closed-listings")
                ? renderLinkToFarmerProfile
                : null}
              ...
              {this.props.match.path !== paths().USER_CLOSED_LISTINGS_PATH ? (
                this.props.listings.length > 0 ? (
                  <h4>
                    {renderListings.length}{" "}
                    {renderListings.length > 1
                      ? "Open Listings"
                      : "Open Listing"}
                  </h4>
                ) : (
...
const mapDispatchToProps = (dispatch, routerProps) => {
  return {
    fetchListings: (id) =>
      dispatch(listingsActions.fetchListings(id, routerProps)),
    fetchCommodities: () => dispatch(fetchCommodities()),
    fetchListing: (id) =>
      dispatch(listingsActions.fetchListing(id, routerProps)),
    ...
    fetchUserClosedListings: (userId) =>
      dispatch(listingsActions.fetchUserClosedListings(userId)),
    fetchUserInterestsListings: (userId) =>
      dispatch(listingsActions.fetchUserInterestsListings(userId)),
    fetchMyInterestsListings: (userId) =>
      dispatch(listingsActions.fetchMyInterestsListings(userId)),
    ...
    deleteListing: (listingId) =>
      dispatch(listingsActions.deleteListing(listingId)),
  };
};
